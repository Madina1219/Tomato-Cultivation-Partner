...
User-centred design started with a primary persona, derived from informal conversations with three first-time growers in London during the design phase (January 2026).

![Maya persona](docs/storyboard/persona_maya.png)

Maya is the **single primary persona** the app is designed for. She represents the largest segment of new UK urban growers: time-poor renters in their late 20s to mid-30s, living in flats with balconies or windowsills, who *want* to grow food but are intimidated by the gap between Instagram garden-aesthetic content and reality. Full persona details, including secondary personas (Greg, 67, retired allotment holder; and Riya, 24, student in halls) live in [`docs/design.md`](docs/design.md).
